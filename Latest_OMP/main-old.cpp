            ...
            if (requiredBiomass.find(i) != requiredBiomass.end()) {//Is the community ID in the array?
                ...
                if (biomass->second > 0){//requisitos distintos a cero
                    if (localidades.find(i) != localidades.end()) { //existe la comunidad con ese numero?
                        ...
                        float tmpCost = 0.0;
                        float timeLimit = 1 * 3600; // 6 hours
...
void parseParameters(int argc, const char** argv){
    try {
        ...
        TCLAP::ValueArg<std::string>friction_w("fw","friction walking","Absolute path to friction.tif",true,"/path/to/friction.tif","string");
        TCLAP::ValueArg<std::string>locs_w("lw","locs walking","Absolute path to locs.tif",true,"/path/to/locs.tif","string");
        TCLAP::ValueArg<std::string>demmand_v("dw","demmand vehicle","Absolute path to demmand.csv",true,"/path/to/demmand.csv","string");
        TCLAP::ValueArg<std::string>friction_v("fv","friction vehicle","Absolute path to friction.tif",true,"/path/to/friction.tif","string");
        TCLAP::ValueArg<std::string>locs_v("lv","locs vehicle","Absolute path to locs.tif",true,"/path/to/locs.tif","string");
        TCLAP::ValueArg<std::string>demmand_w("dv","demmand walking","Absolute path to demmand.csv",true,"/path/to/demmand.csv","string");
        ...
        TCLAP::ValueArg<std::string>timeLimit("t","timeLimit","Time limit for exploration -t <int>",true,"Time limit for exploration -t <int>","string");
        TCLAP::ValueArg<std::string>exp1("e","exponent","IDW exponent -e <float>",true,"IDW exponent -e <float>","string");
        ...
        // Walking
        frictionMapW = friction_w.getValue();
        demmandFileW = demmand_w.getValue();
        locsMapW = locs_w.getValue();

        // Vehicle
        frictionMapV = friction_v.getValue();
        demmandFileV = demmand_v.getValue();
        locsMapV = locs_v.getValue();

        // user defined
        isFriccRelative = stoi(relative.getValue());        
        numThreads = stoi(processors.getValue());
        hours = stoi(timeLimit.getValue());
        exponent = stof(exp1.getValue());
