...
int isFriccRelative = 1, numThreads = 0, hours = 12, skippingYears = -1, startingYear = 2010, finalYear=2050;
...
void parseParameters(int argc, const char** argv){
    try {
        ...
        TCLAP::ValueArg<std::string>sYears("y","skippingYears","Number of skipping years -1 to run all years -y <int>",true,"Number of skipping years -1 to run all years -y <int>","string");
        TCLAP::ValueArg<std::string>starting("s","startingYear","Starting year from which the algorithm will calculate the IDW -s <int>",true,"Starting year from which the algorithm will calculate the IDW <int>","string");
        CLAP::ValueArg<std::string>final("f","finalYear","Final year up to which the algorithm will calculate the IDW -f <int>",true,"Final year up to which the algorithm will calculate the IDW <int>","string");
        ...
        skippingYears = stoi(sYears.getValue());
        startingYear = stoi(starting.getValue());
        finalYear = stoi(final.getValue());
    }
...
void RunCDIDW(string frictionMap, string demmandFile, string locsMap, string scenario){
    ...
    double end2;
    cout << "----------------------------------------------------------" << endl;
    cout << "Running parallel CD and IDW for " << scenario << " scenario" << endl;
    ...
    map<int, Raster::local> localities;// map of localities

    //estructura local contiene el no. de comunidad y su (x,y)
    std::map<int, Raster::local>::iterator ubicacion; //iterados mapa ubicacion localities
    ...
    // Store locs number
    int locsNum = objrast.readLocalities(locsMatrix, rows, cols, localities, nullValue);
    ...
    if (skippingYears == -1)
        skippingYears =1;
    for(int year = 1; year<=demmand.size()-1; year = year + skippingYears){
        ...
        for(int loc=0; loc < demmand[0].second.size();loc++){ // Tamaño de localities
        ...
        for(i=start;i<=end;i++) {
            ...
            if (requiredBiomass.find(i) != requiredBiomass.end()) {//Is the community ID in the array?
                ...
                if (biomass->second > 0){//requisitos distintos a cero
                    if (localities.find(i) != localities.end()) { //existe la comunidad con ese numero?
                        ubicacion = localities.find(biomass->first);//buscar ubicacion de la localidad
                        //ubicaion inicial
                        array.row = ubicacion->second.row;
                        array.col = ubicacion->second.col;
                        array.val_fricc = 0;
                        array.key=0;
                        locCount++;//localities calculadas
        ...
        /*ubicacion = localities.begin();
        while (ubicacion != localities.end()) {
        ...
        // export image
        objrast.exportRaster(IDW_matrix, rows, cols, locsNum, fileName, nullValue);//crea tiff de IDW de todas las localities calculadas
