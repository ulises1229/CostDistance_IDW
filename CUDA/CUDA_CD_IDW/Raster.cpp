...
//-------------------------------------------------------------------------------------------------------------------------
int Raster::readLocalities(float *map_local, int rows, int cols, map<int,local> &local_ord, int cell_null) {
    local array;
    int countLoc=0;
    for(int row=0;row<rows;row++){
        for(int col=0;col<cols;col++){
...
vector<pair<string, vector<float>>> Raster::loadDemmand(string name){
    ...
    while(getline(myFile, line))
    {
        ...
        while(getline(ss, value, ',')){
            ...
            if(colIdx == 0)
                value.erase(remove(value.begin(), value.end(), '"'), value.end()); // remove specia " char.
            // Add value to vector
