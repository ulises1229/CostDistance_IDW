...
public:
	...
	int ROWS, COLS, intervals = 0, totValidGrids = 0, totGridsAvg = 0, valid_points = 0, bGotMin, bGotMax, firstX = 0, firstY = 0;
	bool flag = true;
	...
	float** tiff_to_matrix_gdal(string file, int map) {
		GDALDataset *dataset;
		char **MD;
		char *info;
		GDALAllRegister();
		...
		for (int i = 0; i < ROWS; i++) {
			for (int j = 0; j < COLS; j++) {
				...
				if (cCols == COLS - 1) {
					...
					if(costos[cRows][cCols] > 0 && map == 1){
						valid_points++;
						biomass += costos[cRows][cCols];
						if (stop && costos[cRows][cCols] > 0 && map == 1) {
							firstX = cRows;
							firstY = cCols;
							cout << "fX: " << firstX << ", fY: " << firstY << " FRICC VAL: " << costos[cRows][cCols] << endl; 
							stop = false;
						} else if (map == 2) {
							cout << "Locality at " << cRows << ", " << cCols << ": " << costos[cRows][cCols] << endl;
						}
